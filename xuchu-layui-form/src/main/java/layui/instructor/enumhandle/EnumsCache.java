...
import layui.instructor.utils.ClassScaner;
...
public class EnumsCache {
    ...
    public static final String packageName = "layui.domain";
    public static void init() {
        log.info("EnumsCache init ...");
        ClassScaner.scan(packageName,Enum.class)
                .forEach(clazz -> {
                    if (!clazz.isEnum()) {
                        return;
                    }
                    ...
                    for (Field field : fields) {
                        ...
                        try {
                            ...
                            Method getIdMethod;
                            try {
                                getIdMethod = clazz.getDeclaredMethod("getId");
                            }catch (Exception e){
                                getIdMethod = clazz.getDeclaredMethod("getCode");
                            }
                            if (getIdMethod == null) {
                                continue;
                            }
        ...
        log.info("EnumsCache finish ... {}",cache.toString());
    }
}
